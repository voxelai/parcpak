...
def _download_file(
    url: str, download_dir: str | Path | None = None, overwrite: bool = False
):
...
def fetch_schaefer(version: int, resolution: int = 2):
    ...
    table = _download_file(
        os.path.join(REPO_URL, "tables", f"Schaefer2018_{version}Parcels_7Networks.csv")
    )
...
def fetch_buckner(resolution: int = 2):
    ...
    parc = _download_file(
        os.path.join(
            REPO_URL, "parcellations", f"Buckner7_space-MNI_dseg_{resolution}mm.nii.gz"
        )
    )
